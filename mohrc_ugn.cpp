...
/**
  ...
  @return The function does not return anything, the results are stored in the output
          arguments.
*/
void mohrc_ugn::compute_ordered_trial_spectral(const vector &E_trial,
                                              vector &eig,
                                              vector &Eig_1,
                                              vector &Eig_2,
                                              vector &Eig_3,
                                              ...
                                              matrix &EIG_3) const
{
  vector Eig0_1(ASTCKVEC(NCOMP_STRAIN)), Eig0_2(ASTCKVEC(NCOMP_STRAIN)), Eig0_3(ASTCKVEC(NCOMP_STRAIN));
  matrix metric(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG0_1(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG0_2(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG0_3(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
...
/**
  ...
  @return The function does not return anything, the results are stored in the output
          arguments.
*/
void mohrc_ugn::compute_return_denominators(double &denom_s,
                                           double &denom_l,
...
/**
  ...
  @return The function does not return anything, the results are stored in the output
          arguments.
*/
void mohrc_ugn::compute_returned_principal_stresses(const vector &eig,
                                                   double trace_E,
                                                   int return_type,
                                                   double lambda_s,
                                                   double lambda_l,
...
/**
  ...
  @return This function does not return a value. The computed results are
          ...
          point.
*/
void mohrc_ugn::nlstresses(long ipp, long ido)
{
...
/**
  Computes the actual stresses based on the attained nonlocal strains and state variables.
...
*/
void mohrc_ugn::nonloc_nlstresses(long ipp, long ido)
{
  long ncompstr = Mm->ip[ipp].ncompstr; // the number of stress/strain components at the given integration point
  vector eps(ASTCKVEC(ncompstr));
  vector sig(ASTCKVEC(ncompstr));
...
*/
void mohrc_ugn::nlstresses(const vector &strain,
                          ...
                          vector &statev)
{
  /*
    ...
    Ep(1:3,:) = Ep(1:3,:) + E;
  */
  vector E(ASTCKVEC(NCOMP_STRAIN)), Ep_prev(ASTCKVEC(4)), E_trial(ASTCKVEC(NCOMP_STRAIN));
  vector Ep(ASTCKVEC(4)), eps_el(ASTCKVEC(NCOMP_STRAIN)), eig(ASTCKVEC(3)), sigma(ASTCKVEC(3)), S(ASTCKVEC(NCOMP_STRESS));
...
*/
void mohrc_ugn::matstiff(matrix &d, long ipp, long ido)
{
  long ncompstr = Mm->ip[ipp].ncompstr; // the number of stress/strain components at the given integration point
  vector eps(ASTCKVEC(ncompstr));
  vector sig(ASTCKVEC(ncompstr));
...
*/
void mohrc_ugn::stiffmat(const vector &strain,
                        const vector &statev,
                        const vector &stress,
                        matrix &d)
{
  ...
  */
  vector Ep_prev(ASTCKVEC(4)), E_trial(ASTCKVEC(NCOMP_STRAIN)), eig(ASTCKVEC(3)), sigma(ASTCKVEC(3));
  vector iota(ASTCKVEC(4));
  vector Eig_1(ASTCKVEC(NCOMP_STRAIN)), Eig_2(ASTCKVEC(NCOMP_STRAIN)), Eig_3(ASTCKVEC(NCOMP_STRAIN));
  vector Eig_12(ASTCKVEC(NCOMP_STRAIN)), Eig_23(ASTCKVEC(NCOMP_STRAIN)), Eig_6(ASTCKVEC(NCOMP_STRAIN));
  matrix EIG_1(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG_2(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG_3(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG_12(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
  matrix EIG_23(ASTCKMAT(NCOMP_STRAIN, NCOMP_STRAIN));
...
/**
  The function updates the state variable array eqother from the actual converged iteration values.

  @param[in]  ipp - integration point identfier,
  @param[in]  im  - index of the given material in the material model description array at the given integration point,
  @param[in]  ido - index shift of the state variable array other.
  @return The function does not return anything, the resulting converged state of the given integration
          point other array is stored in the array eqother.
*/
void mohrc_ugn::updateval(long ipp, long im, long ido)
{
  vector statev;
  vector eqstatev;
...
*/
void mohrc_ugn::giveirrstrains (long ipp, long ido, vector &epsp)
{
  long i;
  for (i=0;i<epsp.n;i++)
...
*/
void mohrc_ugn::updateval(const vector &statev, vector &eqstatev)
{
